...
class TestLuminousCinemaVisualGate(unittest.TestCase):
    def test_luminous_cinema_visual_gate(self):
        ...
        (artifact_dir / "luminous_cinema_rendered.html").write_text(html, encoding="utf-8")
        ...
        with sync_playwright() as p:
            ...
            # Assert Hero Video & Poster exist inside section.luminous-hero
            ...
            self.assertIsNotNone(hero_sec, "Missing section.luminous-hero in dark_cinematic.html")
            self.assertIsNotNone(video_el, "Missing video.luminous-hero__video in dark_cinematic.html")
            self.assertIsNotNone(poster_el, "Missing img.luminous-hero__poster in dark_cinematic.html")
            ...
            # Assert hero background color is light/white (#fdfbf7) and NOT dark
            bg_color = page_desktop.eval_on_selector("section.luminous-hero", "el => getComputedStyle(el).backgroundColor")
            self.assertTrue("253" in bg_color or "255" in bg_color, f"Expected light background, got {bg_color}")

            # Assert no page errors
            self.assertEqual(len(page_errors), 0, f"Page error in luminous_cinema: {page_errors}")
            ...
            self.assertEqual(len(page_errors_mobile), 0, f"Mobile page error in luminous_cinema: {page_errors_mobile}")
