...
def generate(csv_path, template, outdir, *, limit=0, city="", only="",
    ...
    leads, order = {}, []
    for r in kept:
        lead = engine.lead_record(r, r["slug"])
        leads[r["slug"]] = lead
        order.append(r["slug"])
    ...
    (dist / "index.html").write_text(
        engine.render_app(tpl_src, leads, order, meta, live=live), encoding="utf-8")

    for i, slug in enumerate(order):
        d = dist / slug
        d.mkdir(parents=True, exist_ok=True)
        (d / "index.html").write_text(
            engine.render_stub(leads[slug], meta, site_name, base_url, live),
            encoding="utf-8")
    ...
    summary = {"template": meta["name"], "built": len(kept), "dropped": len(dropped),
               ...
               "built_at": built_at, "base_url": base_url,
